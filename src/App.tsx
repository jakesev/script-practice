...
import { mergeScripts } from './backup'
...
export function App() {
  ...
  // Import a backup (merge by updatedAt) — local-only, no backend.
  const importScripts = (incoming: Script[]) =>
    setScripts((prev) => {
      const merged = mergeScripts(prev, incoming)
      saveScripts(merged)
      return merged
    })
